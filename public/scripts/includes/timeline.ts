...
function setupTimeline(timeline) {
    ...
    if(width <= 0) dateStr = "Starts in " + daysDiff(nowdate, mindate)
    else if (width < 1) dateStr = "Ends in " + daysDiff(nowdate, maxdate)
    else dateStr = "Ended " + daysDiff(maxdate, nowdate, true) + " ago"
...
function daysDiff(begin: Date, end: Date, past: boolean = false): String {
